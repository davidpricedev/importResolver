...
const _Some = x => ({
  ...
  map: f => _Some(f(x)),
  concat: y => {
    if (isObject(x) && isObject(y._value)) return _Some(merge(x, y._value));
    else if (isObject(x) && isObject(y)) return _Some(merge(x, y));
    else if (isString(x) || isString(y._value)) return _Some(`${x}${y._value}`);
    else if (isArray(x) || isArray(y)) return _Some(concat(x, y));
    else {
  ...
  inspectItemPred: (prefix, pred) => {
    if (pred(x)) inspectItem(mergeText('List', prefix))(x);
